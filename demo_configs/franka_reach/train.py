"""RoboSpec Training Script — RoboSpec-Reach-Franka-v0
...
gym.register(
    ...
    kwargs={
        "env_cfg_entry_point": "franka_panda_reach_env_cfg:FrankaPandaReachEnvCfg",
...
if not os.path.exists(_TRAIN_SCRIPT):
    ...
    print("Install the framework: ./isaaclab.sh --install skrl")
    sys.exit(1)
...
if "--task" not in _user_args:
    _user_args = ["--task", "RoboSpec-Reach-Franka-v0"] + _user_args
if "--num_envs" not in _user_args:
    _user_args = ["--num_envs", "4096"] + _user_args

sys.argv = [_TRAIN_SCRIPT] + _user_args

print(f"{'='*70}")
print(f"  RoboSpec Training: RoboSpec-Reach-Franka-v0")
print(f"  Framework: skrl")
